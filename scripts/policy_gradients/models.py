...
class ValueDenseNet(nn.Module):
    ...
    '''
    def __init__(self, state_dim, init, hidden_sizes=None):
        '''
        ...
        if hidden_sizes == None:                                            # get if the argument hidden_sizes is not None
            global HIDDEN_SIZES                                             # get the global variable hidden size
...
class CtsPolicy(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES,
        ...
        stdev_init = ch.zeros(1, action_dim)                                # modified by including index 1
    ...
    def forward(self, x):
        ...
        log_std = self.log_stdev.expand_as(means)                       # added this new re-implementation of log_std
        # std = ch.exp(self.log_stdev)                      
        std = ch.exp(log_std)

        return means, std, log_std
